...
def _looks_like_mercadolivre(path: Path) -> bool:
    # Usa exatamente a mesma detecção tolerante do importador central:
    # nome do arquivo primeiro e cabeçalhos do relatório como fallback.
    return ShopeeImporter._looks_like_mercadolivre(path)


def save_importation(file_path: str, tipo_periodo: str, data_inicio: date, data_fim: date, mode: str = "perguntar") -> int:
    """Importa e salva desempenho/vendas da Shopee ou Mercado Livre."""
    path = Path(file_path)
    ...
    if _looks_like_mercadolivre(path):
        ...
        result = save_mercadolivre_importation(
            str(path), data_inicio, data_fim,
            replace_same_period=(mode != "somar"),
        )
        return int(result["importacao_id"])

    mes_ref = mes_referencia_from_date(data_inicio)
    lines = ShopeeImporter().preview(path)

    with get_connection() as conn:
        if mode == "substituir_mes":
            existing = conn.execute(
                "SELECT id FROM importacoes WHERE tipo_relatorio = 'performance' AND tipo_periodo = ? AND mes_referencia = ? AND status = 'confirmada'",
            ...
            existing = conn.execute(
                "SELECT id FROM importacoes WHERE tipo_relatorio = 'performance' AND tipo_periodo = ? AND data_inicio = ? AND data_fim = ? AND status = 'confirmada'",
        ...
        cursor = conn.execute(
            ...
            """,
            (path.name, str(path), tipo_periodo, data_inicio.isoformat(), data_fim.isoformat(), mes_ref, now_iso()),
        )
        ...
        for line in lines:
            conn.execute(
                """
                INSERT INTO linhas_importadas (importacao_id, id_item_shopee, produto_nome, id_variacao_shopee, variacao_nome, sku_variacao, vendas_pedido_pago, unidades_pedido_pago, tipo_linha, contabilizar)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (importacao_id, line.id_item_shopee, line.produto_nome, line.id_variacao_shopee, line.variacao_nome, line.sku_variacao, line.vendas_pedido_pago, line.unidades_pedido_pago, line.tipo_linha, 1 if line.contabilizar else 0),
            )
...
def list_importations() -> list[dict]:
    return fetch_all("""
        SELECT id, arquivo_nome, tipo_relatorio, tipo_periodo, data_inicio, data_fim, mes_referencia, status, criado_em
        FROM importacoes ORDER BY criado_em DESC LIMIT 100
    """)
...
def get_importation(importacao_id: int) -> dict | None:
    with get_connection() as conn:
        return conn.execute("""
            SELECT id, arquivo_nome, tipo_relatorio, tipo_periodo, data_inicio, data_fim, mes_referencia, status, criado_em,
                   (SELECT COUNT(*) FROM vendas_contabilizadas v WHERE v.importacao_id = importacoes.id) AS vendas_contabilizadas
            FROM importacoes WHERE id = ?
        """, (importacao_id,)).fetchone()


def delete_importation(importacao_id: int) -> bool:
    with get_connection() as conn:
        current = conn.execute("SELECT id, tipo_relatorio FROM importacoes WHERE id = ?", (importacao_id,)).fetchone()
        if not current:
            return False
...
def _upsert_products_and_sales(conn, importacao_id: int, lines: list[ImportedLine], data_inicio: date, data_fim: date, mes_ref: str) -> None:
    ...
    for line in lines:
        ...
        variacao_id = _upsert_variation(conn, produto_pai_id, line.id_variacao_shopee, line.variacao_nome, line.sku_variacao)
        custo_unitario = _get_current_cost(conn, variacao_id)
        calc = calculate_sale_profit(
            faturamento=line.vendas_pedido_pago, unidades=line.unidades_pedido_pago,
            imposto_percentual=imposto, comissao_percentual=comissao,
            taxa_fixa_unitaria=taxa, custo_unitario=custo_unitario,
        )
        conn.execute("""
        ...
        """, (
            importacao_id, produto_pai_id, variacao_id, data_inicio.isoformat(), data_fim.isoformat(), mes_ref,
            calc.unidades, calc.faturamento, calc.imposto_percentual, calc.comissao_percentual,
            calc.taxa_fixa_unitaria, calc.imposto_valor, calc.comissao_valor, calc.taxa_fixa_valor,
            calc.custo_unitario, calc.custo_total, calc.lucro, 1 if calc.lucro_incompleto else 0, now_iso(),
        ))
...
def _upsert_parent_product(conn, id_item_shopee: str, name: str) -> int:
    existing = None
    if id_item_shopee:
        existing = conn.execute("SELECT id FROM produtos_pai WHERE id_item_shopee = ?", (id_item_shopee,)).fetchone()
    if existing:
        conn.execute("UPDATE produtos_pai SET nome = ? WHERE id = ?", (name, existing["id"]))
        return int(existing["id"])
    cursor = conn.execute("INSERT INTO produtos_pai (id_item_shopee, nome, ativo, criado_em) VALUES (?, ?, 1, ?)", (id_item_shopee or None, name, now_iso()))
...
def _upsert_variation(conn, produto_pai_id: int, variation_id: str, variation_name: str, sku: str) -> int:
    ...
    existing = None
    if variation_id:
        existing = conn.execute("SELECT id FROM variacoes WHERE id_variacao_shopee = ?", (variation_id,)).fetchone()
    else:
        existing = conn.execute("SELECT id FROM variacoes WHERE produto_pai_id = ? AND id_variacao_shopee IS NULL AND nome_variacao = ? ORDER BY id DESC LIMIT 1", (produto_pai_id, variation_name)).fetchone()
    if existing:
        conn.execute("UPDATE variacoes SET produto_pai_id = ?, nome_variacao = ?, sku = COALESCE(NULLIF(?, ''), sku) WHERE id = ?", (produto_pai_id, variation_name, sku, existing["id"]))
        return int(existing["id"])
    cursor = conn.execute("""
...
def _get_current_cost(conn, variacao_id: int) -> float | None:
    row = conn.execute("SELECT custo_unitario FROM custos_variacao WHERE variacao_id = ? AND ativo = 1 ORDER BY criado_em DESC, id DESC LIMIT 1", (variacao_id,)).fetchone()
...
def find_importations_same_period(tipo_periodo: str, data_inicio: date, data_fim: date) -> list[dict]:
    return fetch_all("""
        ...
        WHERE tipo_relatorio = 'performance' AND tipo_periodo = ? AND data_inicio = ? AND data_fim = ? AND status = 'confirmada'
...
def find_importations_same_month(report_type: str, tipo_periodo: str, mes_referencia: str) -> list[dict]:
    return fetch_all("""
        SELECT * FROM importacoes
        WHERE tipo_relatorio = ? AND tipo_periodo = ? AND mes_referencia = ? AND status = 'confirmada'
        ORDER BY criado_em DESC
