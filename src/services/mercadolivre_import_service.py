...
COLUMN_ALIASES = {
    ...
    "unidades": ["Unidades vendidas", "Quantidade de vendas", "Unidades", "Quantidade vendida", "Quantidade", "Itens vendidos"],
...
def _find_header_and_columns(ws):
    # O relatório real do ML possui os cabeçalhos na linha 6.
    # Não usamos ws.max_row porque alguns arquivos têm dimensão indefinida em read_only.
    best = None
    ...
    raise ValueError(
        ...
        "Procurei por Anúncio/Produto, Unidades vendidas/Quantidade de vendas e Vendas brutas (BRL)/Faturamento."
    )
...
def preview_mercadolivre(file_path: str) -> dict:
    ...
    try:
        ...
        for raw_values in ws.iter_rows(min_row=header_row + 1, values_only=True):
            ...
            rows.append({"ad_id": ad_id or f"ML-NOME:{_key(name)}", "produto_nome": name, "variacao_nome": variation or "Sem variação", "sku": sku, "status": status, "unidades": units, "faturamento": gross})
        start, end = _report_period(ws)
        return {"arquivo": path.name, "data_inicio": start, "data_fim": end, "rows": rows, "count": len(rows), "faturamento": sum(r["faturamento"] for r in rows), "unidades": sum(r["unidades"] for r in rows)}
    finally:
        wb.close()
...
def _upsert_ml_product(conn, row: dict) -> tuple[int, int]:
    ...
    if existing:
        ...
        cur = conn.execute("INSERT INTO variacoes (produto_pai_id, id_variacao_shopee, nome_variacao, sku, tipo_produto, ativo, criado_em) VALUES (?, ?, ?, ?, 'pronto', 1, ?)", (parent_id, variation_key, row["variacao_nome"], sku, now_iso()))
        variation_id = int(cur.lastrowid)
    return parent_id, variation_id


def _current_cost(conn, variation_id: int) -> float | None:
    row = conn.execute("SELECT custo_unitario FROM custos_variacao WHERE variacao_id = ? AND ativo = 1 ORDER BY criado_em DESC, id DESC LIMIT 1", (variation_id,)).fetchone()
...
def find_ml_importations_same_period(data_inicio: date, data_fim: date) -> list[dict]:
    return fetch_all("SELECT id, arquivo_nome, data_inicio, data_fim, mes_referencia, criado_em FROM importacoes WHERE tipo_relatorio = ? AND data_inicio = ? AND data_fim = ? AND status = 'confirmada' ORDER BY criado_em DESC", (REPORT_TYPE, data_inicio.isoformat(), data_fim.isoformat()))
...
def save_mercadolivre_importation(file_path: str, data_inicio: date, data_fim: date, replace_same_period: bool = True) -> dict:
    ...
    with get_connection() as conn:
        if replace_same_period:
            existing = conn.execute("SELECT id FROM importacoes WHERE tipo_relatorio = ? AND data_inicio = ? AND data_fim = ? AND status = 'confirmada'", (REPORT_TYPE, data_inicio.isoformat(), data_fim.isoformat())).fetchall()
            for row in existing:
                conn.execute("DELETE FROM importacoes WHERE id = ?", (row["id"],))
        cur = conn.execute("INSERT INTO importacoes (arquivo_nome, caminho_arquivo, tipo_relatorio, tipo_periodo, data_inicio, data_fim, mes_referencia, status, criado_em) VALUES (?, ?, ?, 'personalizado', ?, ?, ?, 'confirmada', ?)", (Path(file_path).name, str(Path(file_path)), REPORT_TYPE, data_inicio.isoformat(), data_fim.isoformat(), mes_referencia_from_date(data_inicio), now_iso()))
